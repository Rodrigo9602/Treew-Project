...
export class ListComponent implements OnInit {
  ...
  /**
   * Estado que controla la visibilidad del submenú de ordenamiento.
   * @type {boolean}
   * @default false
   */
  ...
  /**
   * Referencia al timeout para el ocultamiento del submenú.
   * @private
   * @type {any}
   */
  ...
  handleCreateCard(cardData: { name: string; desc: string }): void {
    this.trelloService
      .createCard(this.list?.id!, cardData.name, cardData.desc)
      .subscribe((newCard: TrelloCard) => {
        this.cardsList.push(newCard);
        this.originalCardsList.push(newCard);
      });
    // evento de notificación
    this.toast.success('Card added', 'The card was successfully added');
  ...
  dropCard(event: CdkDragDrop<any[]>) {
    ...
    this.trelloService
      .updateCardOrder(this.list?.id!, reorderedCards)
      .subscribe({
        next: () => {
  ...
  archiveList() {
    this.trelloService.updateListStatus(this.list?.id!, true).subscribe({
      next: () => {
        this.toast.success(
          'List archived',
          'The list was successfully archived'
        );
  ...
  /**
   * Oculta el submenú con un pequeño delay para mejorar la usabilidad.
   *
   * Permite al usuario mover el cursor entre el elemento padre y el submenú
   * sin que se cierre inmediatamente.
   *
   * @public
   */
  hideSubmenuWithDelay(): void {
  ...
  /**
   * Cancela el ocultamiento del submenú.
   *
   * Se llama cuando el usuario entra al área del submenú,
   * evitando que se cierre mientras navega por las opciones.
   *
   * @public
   */
  ...
  sortBy(option: 'creationDesc' | 'creationAsc' | 'alphabetical' | 'dueDate') {
    ...
    // Cerrar tanto el menú principal como el submenú
    this.showSubmenu = false;
    this.closeMenu();
  }
